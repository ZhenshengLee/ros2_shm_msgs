...
public:
  explicit Listener()
      : Node("intra_gmat_listener", rclcpp::NodeOptions().use_intra_process_comms(true)) {
    ...
    auto callback = [this](const Topic::SharedPtr msg) -> void {
      ...
      m_last_gmatimage->upload(last_cvimage->image);
    ...
    m_subscription = create_subscription<Topic>("intra_gmat", custom_qos_profile, callback);
  }
...
private:
  ...
  std::shared_ptr<cv::cuda::GpuMat> m_last_gmatimage{std::make_shared<cv::cuda::GpuMat>()};
