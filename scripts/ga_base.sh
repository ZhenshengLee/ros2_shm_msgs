...
GA_ROOT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )/.." && pwd )"
...
function set_lib_path() {
  ...
  if [ "$RELEASE_DOCKER" == 1 ]; then
    ...
    PY_LIB_PATH=${GA_ROOT_DIR}/py_proto
    PY_TOOLS_PATH=${GA_ROOT_DIR}/modules/tools
...
function determine_bin_prefix() {
  GA_BIN_PREFIX=$GA_ROOT_DIR
  if [ -e "${GA_ROOT_DIR}/bazel-bin" ]; then
    GA_BIN_PREFIX="${GA_ROOT_DIR}/bazel-bin"
  fi
  export GA_BIN_PREFIX
}
...
function start_prof_customized_path() {
  ...
  LOG="${GA_ROOT_DIR}/data/log/${MODULE}.out"
  is_stopped_customized_path "${MODULE_PATH}" "${MODULE}"
  if [ $? -eq 1 ]; then
    PROF_FILE="/tmp/$MODULE.prof"
    rm -rf $PROF_FILE
    BINARY=${GA_BIN_PREFIX}/modules/${MODULE_PATH}/${MODULE}
    eval "CPUPROFILE=$PROF_FILE $BINARY \
        --flagfile=modules/${MODULE_PATH}/conf/${MODULE}.conf \
        --log_dir=${GA_ROOT_DIR}/data/log $@ </dev/null >${LOG} 2>&1 &"
    sleep 0.5
    is_stopped_customized_path "${MODULE_PATH}" "${MODULE}"
    if [ $? -eq 0 ]; then
...
function start_gdb_customized_path() {
  ...
  eval "gdb --args ${GA_BIN_PREFIX}/modules/${MODULE_PATH}/${MODULE} \
      --flagfile=modules/${MODULE_PATH}/conf/${MODULE}.conf \
      --log_dir=${GA_ROOT_DIR}/data/log $@"
}
...
if [ $GA_IN_DOCKER = "true" ]; then
  ...
  create_data_dir
  set_lib_path $1
  if [ -z $GA_BASE_SOURCED ]; then
    determine_bin_prefix
    export GA_BASE_SOURCED=1
  fi
fi
