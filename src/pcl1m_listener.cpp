...
public:
  explicit Listener(const rclcpp::NodeOptions &options)
      : Node("shm_pcl1m_listener", options) {
    ...
    auto callback = [this](const Topic::SharedPtr msg) -> void {
      ...
      RCLCPP_INFO(get_logger(), "get-pcl1m-transport-time: %.3f", time_offset_ms);
      if(m_last_cloud_ts.nanoseconds() > 0.0)
      {
        RCLCPP_INFO(get_logger(), "get-pcl1m-timestamp_offset-time: %.3f", timestamp_offset_ms);
      }
    ...
    m_subscription = create_subscription<Topic>("shm_pc_1m", qos, callback);
  }
